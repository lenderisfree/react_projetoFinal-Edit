const GridrowText = () => {
  return (
    <div className="bg-graylight paddingverticaldouble marginbottomfull">
      <div className="gridrow">
        <div className="col-12 col-t-10 col-d-8 offset-t-1 offset-d-2">
          <p>
            Nunc a augue velit. Nullam eget velit sit amet orci dignissim
            iaculis. Quisque dignissim neque in odio laoreet sodales. Phasellus
            gravida facilisis est. Ut nec metus lobortis, euismod felis eget,
            ornare erat. Donec gravida diam sed facilisis consequat.
          </p>
        </div>
      </div>
    </div>
  );
};
